...
import xyz.monkeytong.hongbao.utils.BWLog;
...
public class HongbaoService extends AccessibilityService implements SharedPreferences.OnSharedPreferenceChangeListener {
    ...
    public void onAccessibilityEvent(AccessibilityEvent event) {
        ...
        BWLog.d(TAG, "onAccessibilityEvent - event.getEventType()=" + event.getEventType());
    ...
    private void watchChat(AccessibilityEvent event) {
        BWLog.d(TAG, "watchChat - 进入");
        this.rootNodeInfo = getRootInActiveWindow();

        if (rootNodeInfo == null) {
            BWLog.d(TAG, "watchChat - rootNodeInfo=" + rootNodeInfo);
            return;
        }
        ...
        BWLog.d(TAG, "watchChat - mLuckyMoneyReceived=" + mLuckyMoneyReceived +
                ",mLuckyMoneyPicked=" + mLuckyMoneyPicked + ",mReceiveNode=" + mReceiveNode);
        /* 如果已经接收到红包并且还没有戳开 */
        if (mLuckyMoneyReceived && !mLuckyMoneyPicked && (mReceiveNode != null)) {
            BWLog.d(TAG, "watchChat - 红包还未拆开");
            mMutex = true;
        ...
        /* 如果戳开但还未领取 */
        BWLog.d(TAG, "watchChat - mUnpackCount=" + mUnpackCount + ",mUnpackNode=" + mUnpackNode);
        if (mUnpackCount == 1 && (mUnpackNode != null)) {
            BWLog.d(TAG, "watchChat - 拆开还未领取");
            ...
            if (android.os.Build.VERSION.SDK_INT > 23) {
                ...
                dispatchGesture(gestureDescription, new GestureResultCallback() {
                    ...
                    public void onCompleted(GestureDescription gestureDescription) {
                        BWLog.d(TAG, "openPacket - onCompleted");
                        mMutex = false;
                    ...
                    public void onCancelled(GestureDescription gestureDescription) {
                        BWLog.d(TAG, "openPacket - onCancelled");
                        mMutex = false;
    ...
    private boolean watchList(AccessibilityEvent event) {
        BWLog.d(TAG, "watchList - 进入");
        if (mListMutex) {
            BWLog.w(TAG, "watchList - mListMutex=" + mListMutex);
            return false;
        }
        mListMutex = true;
        ...
        if (event.getEventType() != AccessibilityEvent.TYPE_WINDOW_CONTENT_CHANGED || eventSource == null) {
            BWLog.w(TAG, "watchList - 不是一个消息。event.getEventType()=" + event.getEventType() + ",eventSource=" + eventSource);
            return false;
        } else {
            BWLog.d(TAG, "watchList - 窗口内容改变");
        }
        ...
        if (!nodes.isEmpty() && currentActivityName.contains(WECHAT_LUCKMONEY_GENERAL_ACTIVITY)) {
            BWLog.d(TAG, "watchList - nodes.size()=" + nodes.size() + ",currentActivityName=" + currentActivityName);
            AccessibilityNodeInfo nodeToClick = nodes.get(0);
            if (nodeToClick == null) {
                BWLog.d(TAG, "watchList - nodeToClick=" + nodeToClick);
                return false;
            }
            CharSequence contentDescription = nodeToClick.getContentDescription();
            BWLog.d(TAG, "watchList - contentDescription=" + contentDescription);
    ...
    private boolean watchNotifications(AccessibilityEvent event) {
        BWLog.d(TAG, "watchNotifications - 进入");
        // Not a notification
        if (event.getEventType() != AccessibilityEvent.TYPE_NOTIFICATION_STATE_CHANGED) {
            BWLog.d(TAG, "watchNotifications - 不是通知");
            return false;
        }
        ...
        if (!tip.contains(WECHAT_NOTIFICATION_TIP)) {
            BWLog.d(TAG, "watchNotifications - 不是红包 true");
            return true;
        }

        Parcelable parcelable = event.getParcelableData();
        BWLog.d(TAG, "watchNotifications - parcelable=" + parcelable);
        if (parcelable instanceof Notification) {
            Notification notification = (Notification) parcelable;
            try {
                /* 清除signature,避免进入会话后误判 */
                signature.cleanSignature();
                BWLog.d(TAG, "watchNotifications - 清除signature,避免进入会话后误判");
                notification.contentIntent.send();
            } catch (PendingIntent.CanceledException e) {
                e.printStackTrace();
            }
        }
        BWLog.d(TAG, "watchNotifications - 离开 true");
        return true;
    }
    ...
    private void checkNodeInfo(int eventType) {
        BWLog.d(TAG, "checkNodeInfo - 进入 eventType=" + eventType);
        if (this.rootNodeInfo == null) {
            BWLog.d(TAG, "checkNodeInfo - rootNodeInfo=" + rootNodeInfo);
            return;
        }

        BWLog.d(TAG, "checkNodeInfo - signature.commentString=" + signature.commentString);
        ...
        BWLog.d(TAG, "checkNodeInfo - node1=" + node1 + ",currentActivityName=" + currentActivityName);
        if (node1 != null &&
            ...
            BWLog.d(TAG, "checkNodeInfo - excludeWords=" + excludeWords);
            if (this.signature.generateSignature(node1, excludeWords)) {
                mLuckyMoneyReceived = true;
                mReceiveNode = node1;
                BWLog.d(TAG, "checkNodeInfo - signature=" + this.signature.toString());
            }
            return;
        }
        ...
        if (node2 != null && "android.widget.Button".equals(node2.getClassName()) && currentActivityName.contains(WECHAT_LUCKMONEY_RECEIVE_ACTIVITY)) {
            BWLog.d(TAG, "checkNodeInfo - 找到拆红包按钮");
            mUnpackNode = node2;
            mUnpackCount += 1;
            return;
        }
        ...
        if (mMutex && eventType == AccessibilityEvent.TYPE_WINDOW_STATE_CHANGED && hasNodes
            ...
            BWLog.d(TAG, "checkNodeInfo - 判断红包是否已抢完");
            mMutex = false;
            mLuckyMoneyPicked = false;
            mUnpackCount = 0;
